/**
 * CreateCampaignModal - Simplified campaign creation
 *
 * Just select a client and create - that's it!
 * Campaign is created with LEAD status, auto-generated name.
 * All other details can be added on the campaign detail page.
 */
...
import { useCreateCampaign, useCampaigns } from '@/api/hooks/useCampaigns'
...
export function CreateCampaignModal({ open, onOpenChange }: CreateCampaignModalProps) {
  ...
  // Get recent campaigns to show clients with active campaigns
  const { data: recentCampaignsData } = useCampaigns({
    ordering: '-created_at',
    page_size: 10,
  })

  const recentCampaigns = recentCampaignsData?.results || []

  // Get unique recent clients from campaigns
  const recentClients = Array.from(
    new Map(
      recentCampaigns.map(c => [c.client.id, c.client])
    ).values()
  ).slice(0, 5)
  ...
  const handleClientSelect = async (client: Entity) => {
    ...
    try {
      ...
      const campaignName = `${client.display_name} - ${monthYear}`

      // Create campaign immediately with LEAD status
      const campaign = await createCampaign.mutateAsync({
        name: campaignName,
        client_id: client.id,
        campaign_type: 'external', // Default to external, can be changed later
      })
  ...
  // Show recent clients when not searching (filter out any invalid clients)
  const validRecentClients = recentClients.filter(client => client?.display_name)
  const showRecentClients = !searchQuery && validRecentClients.length > 0
        ...
        <div className="space-y-4">
          ...
          <div className="pt-2 border-t">
            <p className="text-xs text-muted-foreground text-center">
              Select a client to create a new campaign. You can add platforms and details after.
            </p>
          </div>
        </div>
      </DialogContent>
